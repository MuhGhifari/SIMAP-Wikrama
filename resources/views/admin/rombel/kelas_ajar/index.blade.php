 ...
 $(function () {
  ...
  $.ajaxSetup({
	...
	$('body').on('click', '.editData', function () {
		...
		window.location = "{{ route('admin.kelas-ajar') }}" + "/" + data_id + "/rombel";
	 });
		...
		if (rombel.length == 0 || tingkatan.length == 0 || jurusan.length == 0 || tahun_ajaran.length == 0) {
		  ...
		  $('#saveBtn').html('Simpan');	
			...
			$.ajax({
			  ...
			  url: "{{ route('admin.kelas-ajar.store') }}",
			  type: "POST",
			  dataType: 'json',
	...
	$('body').on('click', '.deleteData', function () {
		...
		if (v == true) {
			$.ajax({
				type: "DELETE",
				url: "{{ route('admin.kelas-ajar') }}" +'/' + data_id +'/hapus',
				success: function (data) {
					table.draw();
